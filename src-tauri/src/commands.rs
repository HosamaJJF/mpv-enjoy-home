...
use crate::domain::{
    ...
    RemoteLibraryEntry, RemoteMediaDetail, UpdateApplyResult, UpdateCheckResult,
};
...
#[tauri::command]
pub async fn check_app_update(state: State<'_, AppState>) -> Result<UpdateCheckResult, String> {
    let service = state.service.clone();
    tauri::async_runtime::spawn_blocking(move || service.check_app_update())
        .await
